...
(function () {
    ...
    if (mbwayPhoneInput) {
        ...
        // ✅ Copiar telemóvel principal para MBWay automaticamente EM TEMPO REAL
        phoneInput.addEventListener('input', function () {
    ...
    paymentTabs.forEach(tab => {
        tab.addEventListener('click', function () {
            ...
            // 🎨 Aplicar cores APENAS nos CARDS de pagamento
            ...
            // ❌ NÃO gerar referência no checkout - só na página individual
        });
    });
    ...
    async function processMBWay() {
        ...
        try {
            ...
            if (result.success) {
                // LOG ORDER TO ADMIN
                await logOrder('mbway', 12.90, { name: fullName, email, phone: mbwayPhone, document: nif });
    ...
    async function processMultibanco() {
        ...
        try {
            ...
            if (result.success) {
                // LOG ORDER TO ADMIN
                await logOrder('multibanco', 12.90, { name: fullName, email, phone: phone, document: nif });
    ...
    // 🎨 Aplicar cores ao carregar a página (MBWay já vem selecionado)
    const mbwayTab = document.querySelector('[data-method="mbway"]');
