...
(function () {
    ...
    // --- DISCOUNT LOGIC (RECOVERY) ---
    const urlParams = new URLSearchParams(window.location.search);
    const hasDiscount = urlParams.get('discount') === 'true';
    const basePrice = hasDiscount ? 7.99 : 12.90;

    // Initial UI for Discount
    if (hasDiscount) {
        // Find existing price elements and update visuals
        const orderItemPrice = document.querySelector('.item-price .price-current');
        if (orderItemPrice) orderItemPrice.textContent = '7,99€';

        const oldPrice = document.querySelector('.item-price .price-old');
        if (oldPrice) oldPrice.style.textDecoration = 'line-through';

        // Add a small badge
        const summaryHeader = document.querySelector('.summary-header h3');
        if (summaryHeader) {
            summaryHeader.innerHTML = 'Resumo <span style="background: #10B981; color: white; padding: 2px 6px; border-radius: 4px; font-size: 10px; margin-left: 5px;">OFERTA ATIVA</span>';
        }
    }
    ...
    function showInputError(input, message) {
        ...
        window.showErrorModal('Campo Inválido', message);
        // input.focus(); // Evitar foco agressivo com modal
    ...
    function validateForm() {
        ...
        const nif = nifInput.value.replace(/\D/g, '');
        const phone = phoneInput.value.replace(/\D/g, '');

        if (fullName.length < 3) {
            showInputError(fullNameInput, 'Por favor, insere o teu nome completo.');
            return false;
        }

        if (!email.match(/^[^\s@]+@[^\s@]+\.[^\s@]+$/)) {
            showInputError(emailInput, 'Por favor, insere um email válido e obrigatório.');
            return false;
        }

        if (nif.length !== 9) {
            showInputError(nifInput, 'Por favor, insere um NIF válido (9 dígitos).');
            return false;
        }

        if (phone.length !== 9) {
            showInputError(phoneInput, 'Por favor, insere um número de telemóvel válido (9 dígitos).');
            return false;
        }
        ...
        if (selectedPaymentMethod === 'mbway') {
            ...
            if (mbwayPhone.length !== 9) {
                showInputError(mbwayPhoneInput, 'Por favor, insere o número de telemóvel MBWay (9 dígitos).');
                return false;
            }
        }
    ...
    async function processMBWay() {
        ...
        try {
            ...
            // Metadata with Coupon
            const metaData = { bumps };
            if (hasDiscount) {
                metaData.coupon = 'RECUPERACAO_799';
                metaData.utm_source = 'email_recovery';
            }
            ...
            const result = await WayMBService.createTransaction({
                ...
                meta: metaData // Enviar meta atualizado
            });
            ...
            // Tratamento Específico MBWay (400/500)
            let errorMsg = error.message || 'Erro ao processar pagamento';
            let errorTitle = 'Pagamento Recusado';

            // Detectar erros típicos de número inválido/sem MBWay
            if (errorMsg.includes('400') || errorMsg.includes('500') || errorMsg.includes('Bad Request') || errorMsg.toLowerCase().includes('mbway')) {
                errorMsg = `Não foi possível enviar o pedido de pagamento.<br><br>Por favor, verifique se o número <strong style="color:white">${mbwayPhone}</strong> tem o <strong>MB WAY</strong> ativo e tente novamente.`;
                errorTitle = 'MB WAY Não Encontrado';
            }
            ...
            // Mostrar Modal
            window.showErrorModal(errorTitle, errorMsg);

            // Ocultar status inline se houver
    ...
    function showStatus(title, message, type = 'success') {
        ...
        if (type === 'loading') {
            ...
            if (type === 'error') {
                window.showErrorModal('Erro', message);
            }
        }
    }
    ...
    // Capturar parâmetros da URL (Já capturado no início)
    ...
    window.showErrorModal = function (title, message) {
        ...
        if (modal && modalTitle && modalMessage) {
            ...
            // Allow HTML for bold text
            modalMessage.innerHTML = message || 'Ocorreu um erro inesperado.';
